...
class RpcClientSocket {
    ...
    doConnect(delay) {
        ...
        this.connectTimer = setTimeout(() => {
            let connectCb = function () {
                self.app.logger("frame" /* loggerType.frame */, "info" /* loggerLevel.info */, `rpcClient -> connect to rpc server success: ${self.id}`);
                // register
            ...
            self.app.logger("frame" /* loggerType.frame */, "info" /* loggerLevel.info */, `rpcClient -> try to connect to rpc server: ${self.id}`);
        }, delay);
    }
    onClose() {
        ...
        this.heartbeatTimeoutTimer = null;
        this.socket = null;
        this.app.logger("frame" /* loggerType.frame */, "error" /* loggerLevel.error */, `rpcClient -> socket closed, reconnect the rpc server later: ${this.id}`);
    ...
    heartbeatTimeoutStart() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
            self.app.logger("frame" /* loggerType.frame */, "error" /* loggerLevel.error */, `rpcClient -> heartbeat timeout, close the rpc socket: ${self.id}`);
            self.socket.close();
