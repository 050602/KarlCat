...
class TickTask extends Sigleton_1.Sigleton {
    constructor() {
        super(...arguments);
        this.TaskDic = new Map(); //Key 时间戳， Value 任务 [上下文，回调方法，参数]
        this.queueArr = []; //执行任务的时间戳 队列
        this._pause = false; //是否暂停Tick
        this.isRuning = false; //是否运行中
    }
    static get Instance() {
    ...
    pushTask(thisObj, callback, timestamp, ...data) {
        ...
        //任务已过期
        if (isNaN(timestamp) || timestamp < DateUtils_1.DateUtils.timestamp()) {
            console.error("时间戳非法/该任务已过期", timestamp, DateUtils_1.DateUtils.timestamp());
            // let date = new Date();
        ...
        newarr.push(data);
        console.error("pushTask", newarr, timestamp, DateUtils_1.DateUtils.formatFullTime4(timestamp));
        //从字典获取该时间戳是否存在任务队列
    ...
    hasTask(thisObj, func, timestamp) {
        let arr = this.TaskDic.get(timestamp);
        if (arr) {
            for (let i = 0; i < arr.length; i++) {
                let obj = arr[i][0];
    ...
    update() {
        ...
        if (this.queueArr.length > 0) {
            let curServerTime = DateUtils_1.DateUtils.timestamp();
            this.checkTask(this.queueArr[0], curServerTime);
        }
    }
    checkTask(taskTime, curServerTime) {
